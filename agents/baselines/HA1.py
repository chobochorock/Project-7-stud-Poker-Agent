...
class HA1PokerAgent(PokerAgent):
    ...
    def choose_action(
        ...
        if "CHECK" in valid:
            ...
            if edge >= 0.32:
                action = self._first_valid(
                    valid, ("FULL", "HALF", "QUARTER", "BBING", "CHECK")
                )
            elif edge >= 0.20:
            ...
            if edge < 0:
                ...
                action = self._first_valid(
                    valid, ("FULL", "HALF", "QUARTER", "DDADANG", "CALL")
                )
            elif edge >= 0.24:
                action = self._first_valid(
                    valid, ("HALF", "QUARTER", "DDADANG", "CALL")
                )
            elif edge >= 0.12:
    ...
    def choose_discard_and_reveal(self, hidden_cards: Sequence[Any]) -> tuple[int, int]:
        ...
        reveal_idx = max(
            retained_indices, key=lambda index: self._reveal_value(cards[index], cards)
        )
    ...
    def estimate_equity(self, state: dict[str, Any]) -> float:
        ...
        public_by_opponent = {
            opponent.get("seat", f"opponent_{index}"): self._cards(
                opponent.get("public_cards", [])
            )
    ...
    def estimate_hand_range(
        ...
        return estimate_uniform_hand_range(
            state, samples_per_hand=samples_per_hand, seed=seed
        )
    ...
    def _initial_equity(
        self, retained: list[Card], all_initial_cards: list[Card]
    ) -> float:
    ...
    def _unknown_dead_card_count(
        self, state: dict[str, Any], opponents: Sequence[dict[str, Any]]
    ) -> int:
        count = 0
            ...
            if opponent.get("is_folded"):
                fold_street = next(
                    (
                        ...
                        if event.get("actor") == opponent.get("seat")
                        and event.get("action") == "FOLD"
                    ),
                    None,
                )
    ...
    def _first_valid(
        self, valid_actions: Sequence[str], preferences: Sequence[str]
    ) -> str:
