...
class HeuristicPokerAgent(PokerAgent):
    ...
    def choose_discard_and_reveal(self, hidden_cards: Sequence[Any]) -> tuple[int, int]:
        if len(hidden_cards) < 2:
            raise ValueError(
                "At least two hidden cards are needed to discard and reveal."
            )
        ...
        discard_idx = min(
            range(len(cards)),
            key=lambda index: (
                self._keep_value(cards[index], cards),
                cards[index].value,
            ),
        )
        reveal_candidates = [
            index for index in range(len(cards)) if index != discard_idx
        ]
        reveal_idx = max(
            reveal_candidates,
    ...
    def _choose_free_action(self, strength: float, valid_actions: Sequence[str]) -> str:
        if strength >= 0.90:
            return self._first_valid(
                valid_actions, ("FULL", "HALF", "QUARTER", "BBING", "CHECK")
            )
        if strength >= 0.76:
            return self._first_valid(
                valid_actions, ("HALF", "QUARTER", "BBING", "CHECK")
            )
        if strength >= 0.58:
    ...
    def _choose_facing_bet_action(
        ...
        if strength >= 0.90:
            return self._first_valid(
                valid_actions, ("HALF", "QUARTER", "DDADANG", "CALL", "FOLD")
            )
        if strength >= 0.76 and self._call_pressure(state) < 0.55:
            return self._first_valid(
                valid_actions, ("QUARTER", "DDADANG", "CALL", "FOLD")
            )
    ...
    def _own_cards(self, state: dict[str, Any]) -> list[Card]:
        return self._cards_from_labels(
            state.get("my_hidden_cards", [])
        ) + self._cards_from_labels(state.get("my_public_cards", []))
