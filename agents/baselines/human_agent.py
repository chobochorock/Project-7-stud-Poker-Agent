...
class HumanAgent(BasePokerAgent):
    def choose_action(
        ...
        print(f"\n[{self.name}]")
        print(
            f"chips={state['my_chips']} pot={state['pot']} call={state['call_amount']}"
        )
    ...
    def choose_discard_and_reveal(self, hidden_cards: Sequence[Any]) -> tuple[int, int]:
        ...
        while True:
            try:
                ...
                continue
            if (
                discard_idx != reveal_idx
                and 0 <= discard_idx < len(hidden_cards)
                and 0 <= reveal_idx < len(hidden_cards)
            ):
    ...
    def learn_from_database(
        ...
        return {
            ...
            "trained": False,
            "reason": "Human agents do not train.",
        }
...
class WebHumanAgent(BasePokerAgent):
    ...
    def choose_discard_and_reveal(self, hidden_cards: Sequence[Any]) -> tuple[int, int]:
        raise RuntimeError(
            "Web human discard/reveal choices are submitted through the browser."
        )
    ...
    def learn_from_database(
        ...
        return {
            "agent": type(self).__name__,
            "trained": False,
            "reason": "Human web agents do not train.",
        }
