...
class ClusterPokerAgent(PokerAgent):
    ...
    def __init__(
        ...
        *,
        ...
        if (
            decision == "policy"
            and float(checkpoint.get("policy_loss_weight", 1.0)) == 0
        ):
            raise ValueError(
                "This model was trained without a valid policy target; use decision='q'."
            )
    ...
    def choose_action(
        ...
        if state.get("game_mode") != "ev" or state.get("seat_count") != 2:
            raise ValueError(
                "Cluster agent only supports stackless heads-up EV states."
            )
        ...
        if self.clusterer == "raw":
            if self.decision == "q":
                ...
                action = ACTION_NAMES[
                    self.rng.choices(
                        legal_indices.tolist(), weights=probabilities, k=1
                    )[0]
                ]
        else:
    ...
    def diagnostics(self) -> dict[str, object]:
        ...
        if self.clusterer == "raw":
            return {
                ...
                "action_counts": {
                    key: value for key, value in self.action_counts.items() if value
                },
            }
    ...
    def _outputs(
        ...
        scalars = np.asarray(
            [
                [math.log1p(float(payload[index]) / ante) for index in range(4, 10)]
    ...
    def _responsibility(self, latent: np.ndarray) -> np.ndarray:
        if self.clusterer == "kmeans":
            ...
            scores = (
                normalized
                @ self.artifact["centers"].T
                / float(self.artifact["temperature"])
            )
        else:
            whitened = (latent - self.artifact["whitening_mean"]) @ self.artifact[
                "whitening_transform"
            ]
