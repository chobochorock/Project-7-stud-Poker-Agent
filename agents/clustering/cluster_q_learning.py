    ...
    def __init__(
        ...
        *,
        ...
        if self.q_table.shape != self.component_q.shape:
            raise ValueError(
                "Shared Q-table shape does not match the cluster artifact."
            )
    ...
    def choose_action(
        ...
        if self.rng.random() < self.epsilon:
            ...
            action_index = self.rng.choice(
                legal[np.isclose(scores[legal], best)].tolist()
            )
...
def run_cluster_q_learning(
    ...
    *,
    ...
    if hands <= 0 or ante <= 0 or progress_hands < 0:
        raise ValueError(
            "hands and ante must be positive; progress_hands must be non-negative."
        )
    ...
    for hand_index in range(hands):
        ...
        if progress_hands and (hand_index + 1) % progress_hands == 0:
            ...
            print(
                f"{hand_index + 1:,}/{hands:,} hands ({(hand_index + 1) / elapsed:,.1f}/s)"
            )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run TD(0) self-play on a cluster Q-table."
    )
