...
ACTIONS = np.asarray(("CHECK", "BBING", "DDADANG", "QUARTER", "HALF", "CALL", "FOLD"))
...
def _analyze_artifact(path: Path, kind: str) -> dict[str, object]:
    ...
    with np.load(path) as artifact:
        ...
        q_top = q_order[:, -1]
        q_margin = (
            component_q[np.arange(len(support)), q_top]
            - component_q[np.arange(len(support)), q_order[:, -2]]
        )
        ...
        if kind == "spherical_kmeans":
            ...
            geometry_summary = {"nearest_cosine_quantiles": _quantiles(nearest_value)}
        else:
            ...
            rows.append(
                {
                    ...
                    "q_range": float(
                        component_q[index].max() - component_q[index].min()
                    ),
                    "policy_top_action": str(ACTIONS[policy_top[index]]),
                    "policy_top_probability": float(
                        component_policy[index, policy_top[index]]
                    ),
