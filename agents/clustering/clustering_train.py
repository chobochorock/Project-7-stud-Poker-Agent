...
PAD = 255
RAW_FEATURES = (
    CARD_SLOTS * 18 + MAX_HISTORY * (6 + ACTION_COUNT) + 4 + 2 + ACTION_COUNT + 7
)
...
def load_uct_dataset(
    ...
    *,
    ...
    for input_path in paths:
        ...
        with sqlite3.connect(uri, uri=True) as connection:
            ...
            for row in connection.execute(f"{sql} LIMIT ?", parameters + (remaining,)):
                ...
                q_scale = (
                    max(ante, float(payload[4])) if q_normalization == "pot" else ante
                )
                q[index, visited] = (
    ...
    print(
        f"dataset: {rows:,} rows ({train_rows:,} train, {validation_rows:,} validation)"
    )
    return UCTDataset(
        cards,
        history,
...
def _batch(
    data: UCTDataset, indices: np.ndarray, device: torch.device
    ...
    *,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for start in range(0, len(shuffled), batch_size):
            ...
            policy_loss = (
                -(batch["policy"] * F.log_softmax(masked_logits, dim=1)).sum(1).mean()
            )
...
def fit_gmm(
    ...
    *,
    ...
    weights, means, variances, converged, iterations, lower_bound = _fit_diagonal_gmm(
        whitened_fit,
        clusters=clusters,
        device=device,
...
def _component_stats(
    ...
    global_q = (data.q[indices] * np.sqrt(data.visits[indices].astype(np.float64))).sum(
        axis=0
    ) / np.maximum(global_q_weight, 1e-12)
...
def _add_metrics(
    ...
    legal = ((data.legal_mask[indices, None] >> np.arange(ACTION_COUNT)) & 1).astype(
        bool
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("agents/clustering/data/models/clustering_v1"),
    )
    ...
    parser.add_argument(
        "--max-rows", type=int, default=0, help="0 uses every matching row."
    )
