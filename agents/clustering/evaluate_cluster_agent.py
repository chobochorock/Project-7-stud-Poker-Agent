...
def _create_agent(
    ...
    *,
    ...
    if agent_type == "mccfr-plus-table":
        agent = MCCFRPokerAgent(name, iterations=0, seed=seed, regret_plus=True)
    ...
    if agent_type == "mccfr-6plus-table":
        agent = MCCFRPokerAgent(name, iterations=0, seed=seed, start_street="6th")
...
def evaluate_match(
    ...
    *,
    ...
    result: dict[str, Any] = {
        ...
        "model_dir_b": str(
            (model_dir if model_dir_b is None else model_dir_b).resolve()
        ),
        "hands": hands,
