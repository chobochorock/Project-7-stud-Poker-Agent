    ...
    def test_three_baselines_train_from_uct_database(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for index in range(64):
                street = index % 4
                visits = (
                    base_visits[index % len(base_visits) :]
                    + base_visits[: index % len(base_visits)]
                )
                state = json.dumps(
                    [
                        ...
                        index % 6,
                        [
                            [street, event % 2, event % len(action_names)]
                            for event in range(index % 5)
                        ],
                    ],
                    separators=(",", ":"),
                )
                means = [
                    (action + 1) * 0.1 + (index % 3) * 0.05
            ...
            for clusterer in ("raw", "kmeans", "gmm"):
                for decision in ("policy", "q"):
                    ...
                    self.assertIn(
                        agent.choose_action(state, valid_actions), valid_actions
                    )
