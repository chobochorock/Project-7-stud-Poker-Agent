...
def estimate_uniform_hand_range(
    ...
    for hidden_pair in possible_hands:
        ...
        for _ in range(samples_per_hand):
            ...
            opponent_final = (
                list(opponent_public) + list(hidden_pair) + sampled[cursor:]
            )
        ...
        if samples_per_hand > 1:
            ...
            sample_variance = max(
                0.0,
                (share_square_sum - samples_per_hand * equity * equity)
                / (samples_per_hand - 1),
            )
    ...
    return {
        ...
        "opponent_hand_categories": {
            category: total_categories[category] / total_samples
