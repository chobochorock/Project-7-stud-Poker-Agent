...
DEFAULT_EXE = ROOT / "agents/cpp_mccfr" / "stud_mccfr.exe"
DEFAULT_ATLAS = ROOT / "agents/cpp_mccfr" / "power64_v1.bin"
DEFAULT_MODEL = ROOT / "agents/cpp_mccfr" / "root_mccfr_ante1000_100m.bin"
if not DEFAULT_MODEL.is_file():
    DEFAULT_MODEL = ROOT / "agents/cpp_mccfr" / "root_mccfr_current_snapshot.bin"
if not DEFAULT_MODEL.is_file():
    DEFAULT_MODEL = ROOT / "agents/cpp_mccfr" / "root_mccfr_ante1000_10m.bin"
PBS_EXE = ROOT / "agents/cpp_mccfr" / "stud_mccfr_pbs_resolver.exe"
PBS_ATLAS = ROOT / "agents/cpp_mccfr" / "power512_epsheur20_memory16_v1.bin"
PBS_MODEL = (
    ROOT / "agents/cpp_mccfr" / "made_call_r1000_k512_epsheur20_memory16_30m.bin"
)
...
class CppMCCFRAgent(BasePokerAgent):
    ...
    def __init__(
        ...
        *,
        ...
        bucket: str = "power",
        seventh_resolve_iterations: int = 0,
        seventh_resolve_prior: float = 100.0,
        seventh_posterior_particles: int = 0,
        seventh_hand_history: bool = False,
        seed: int | None = None,
    ):
        super().__init__(name)
        ...
        self.bucket = bucket
        self.seventh_resolve_iterations = seventh_resolve_iterations
        self.seventh_posterior_particles = seventh_posterior_particles
        ...
        if seventh_resolve_iterations:
            command.extend(
                [
                    "--seventh-resolve-iterations",
                    str(seventh_resolve_iterations),
                    "--seventh-resolve-prior",
                    str(seventh_resolve_prior),
                    "--seventh-posterior-particles",
                    str(seventh_posterior_particles),
                ]
            )
            if seventh_hand_history:
                command.append("--seventh-hand-history")
    ...
    def choose_action(
        ...
        if (
            len(response) != 2
            or response[0] != "ACTION"
            or response[1] not in valid_actions
        ):
    ...
    def diagnostics(self) -> dict[str, Any]:
        return {
            ...
            "bucket": self.bucket,
            "seventh_resolve_iterations": self.seventh_resolve_iterations,
            "seventh_posterior_particles": self.seventh_posterior_particles,
    ...
    def _ask(self, command: str) -> str:
        with self._lock:
            if self._process.poll() is not None:
                raise RuntimeError(
                    self._process.stderr.read().strip() or "C++ agent stopped."
                )
...
class CppPBSSeventhAgent(CppMCCFRAgent):
    """30M blueprint with a PBS-conditioned exact-history 7th resolver."""
    ...
    def __init__(self, name: str, *, seed: int | None = None):
        super().__init__(
            ...
            seventh_resolve_iterations=10_000,
            seventh_resolve_prior=100.0,
            seventh_posterior_particles=64,
            seventh_hand_history=True,
            seed=seed,
        )
