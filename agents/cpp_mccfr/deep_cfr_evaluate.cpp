...
public:
    ...
    std::array<double, kActionCount> policy(
        const State& state, int actor, bool* found = nullptr) const {
        if (found) *found = true;
        return client_.strategy(state, actor);
    }
...
private:
    mutable AdvantageClient client_;
    std::mt19937_64 rng_;
};
...
int main(int argc, char** argv) {
    try {
        ...
        const std::string opponent = argument(
            argc, argv, "--opponent", "heuristic");
        const int particles = std::stoi(argument(
            argc, argv, "--belief-particles", "240"));
        const uint64_t seed = std::stoull(argument(argc, argv, "--seed", "7"));
        if (hands <= 0 || hands % 2 || ante <= 0 || stack_ante <= 0 ||
            start_street < 5 || start_street > 7 || particles <= 0 ||
            (opponent != "heuristic" && opponent != "policy-lbr")) {
        ...
        if (opponent == "policy-lbr") {
            if (start_street != 5) {
                throw std::runtime_error("policy-lbr evaluation currently requires start-street 5");
            }
            Options options;
            options.hands = hands;
            options.ante = ante;
            options.stack_ante = stack_ante;
            options.start_street = start_street;
            options.seed = seed;
            options.belief_particles = particles;
            options.report_every = std::stoi(argument(
                argc, argv, "--report-every", "0"));
            options.progress_seconds = std::stod(argument(
                argc, argv, "--progress-seconds", "30"));
            return run_policy_lbr(
                options, policy, "deep-cfr-stud-policy");
        }
        ...
        for (int hand = 0; hand < hands; hand += 2) {
            ...
            const double first = play_hand(
                deck, 0, policy, 0, ante, start_street, -1, nullptr, stack_ante);
            const double second = play_hand(
                deck, 1, policy, 0, ante, start_street, -1, nullptr, stack_ante);
