...
def serve(args: argparse.Namespace) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server:
        ...
        server.listen(1)
        print(
            json.dumps({"server": "ready", "host": args.host, "port": args.port}),
            flush=True,
        )
        ...
        with connection:
            while True:
                ...
                if model is None and seat_models is None:
                    ...
                    model = (
                        make_model(dimensions, args.hidden, args.layers)
                        .to(device)
                        .eval()
                    )
                ...
                if output.shape != (batch_size, ACTIONS):
                    raise RuntimeError(
                        f"model returned {tuple(output.shape)}, expected {(batch_size, ACTIONS)}"
                    )
