...
def evaluate(
    *,
    ...
    heuristic_agents = {name: HeuristicPokerAgent(name) for name in PLAYER_NAMES}
    ...
    try:
        for deal_index in range(deals):
            ...
            for rotation in range(rotations):
                target_seats = {
                    (rotation + offset) % 5 for offset in range(target_count)
                }
                agents = {}
                ...
                target_profit = (
                    sum(final[PLAYER_NAMES[seat]] for seat in target_seats)
                    / target_count
                    / ante
                )
    ...
    return {
        ...
        "hands": hands,
        "ante": ante,
        "effective_stack_ante": (ev_stack_ante if stack_min_ante is None else None),
        "stack_sampling": (
            {
        ...
        "target_profit_by_seat_ante": {
            name: _summary(values) for name, values in seat_values.items() if values
        },
        ...
        "agent_diagnostics": {
            name: agent.diagnostics() for name, agent in cpp_agents.items()
        },
    }
