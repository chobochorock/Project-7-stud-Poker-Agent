...
def create_agent(
    agent_type: str, name: str, simulations: int, seed: int
) -> BasePokerAgent:
    if agent_type == "ha1":
...
def evaluate_heads_up(
    ...
    for hand_index in range(hands):
        ...
        agents = {
            ...
            b_seat: create_agent(
                agent_b_type, b_seat, simulations, paired_seed * 2 + 1
            ),
        }
        game = PokerGame(
            ["Player_1", "Player_2"],
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a seat-paired cash-game heads-up evaluation."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="Monte Carlo samples per HA1 decision",
    )
...
def main() -> None:
    ...
    print(
        f"Chip net: {result['total_profit']:+d} ({result['average_profit']:+.3f}/hand)"
    )
