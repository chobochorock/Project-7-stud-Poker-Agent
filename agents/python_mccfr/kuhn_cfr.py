...
class Node:
    ...
    def strategy(self, reach: float) -> tuple[float, float]:
        ...
        total = sum(positive)
        strategy = (
            (positive[0] / total, positive[1] / total) if total > 0.0 else (0.5, 0.5)
        )
...
def expected_value(strategy: dict[str, tuple[float, float]]) -> float:
    def walk(cards: tuple[str, str], history: str) -> float:
        ...
        return sum(
            probabilities[action] * walk(cards, history + ("p" if action == 0 else "b"))
            for action in range(2)
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train exact tabular CFR on Kuhn poker."
    )
