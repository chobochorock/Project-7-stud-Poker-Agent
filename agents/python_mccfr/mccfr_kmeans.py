...
def _cluster_allocation(group_sizes: dict[str, int], total: int) -> dict[str, int]:
    if total < len(group_sizes):
        raise ValueError(
            f"clusters must be at least the {len(group_sizes)} history groups"
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Compress a 7th-street MCCFR table with k-means."
    )
    parser.add_argument(
        "--input",
        type=Path,
        default=Path("agents/python_mccfr/data/models/mccfr_7th.json"),
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("agents/python_mccfr/data/models/mccfr_7th_kmeans.json"),
    )
