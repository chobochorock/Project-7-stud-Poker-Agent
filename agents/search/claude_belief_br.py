...
class ClaudeBeliefBRAgent(PokerAgent):
    ...
    def choose_action(
        ...
        best_action = (
            max(passive, key=lambda a: evs[a])
            if passive
            else max(valid, key=lambda a: evs[a])
        )
        if aggressive:
    ...
    def choose_discard_and_reveal(self, hidden_cards: Sequence[Any]) -> tuple[int, int]:
        ...
        reveal_idx = max(
            retained, key=lambda index: self._reveal_value(cards[index], cards)
        )
    ...
    def estimate_belief(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        own_cards = self._cards(state.get("my_hidden_cards", [])) + self._cards(
        ...
        active = [
            o
            for o in opponents
            if not o.get("is_folded") and not o.get("is_eliminated")
        ]
        ...
        if not own_cards or not active:
            return {
                **blank,
                "equity": 1.0 if own_cards and not active else fair_share,
        ...
        opp_hidden_counts = [
            self._opponent_hidden_count(state, public) for public in opp_publics
        ]
        opp_future_counts = [
            max(0, 7 - len(public) - hidden)
            for public, hidden in zip(opp_publics, opp_hidden_counts)
        ]
        my_needed = max(0, 7 - len(own_cards))
        dead_count = len(active)  # each live opponent discarded exactly one card at H4
        sample_size = (
            dead_count + my_needed + sum(opp_hidden_counts) + sum(opp_future_counts)
        )
        ...
        for _ in range(self.belief_particles):
            ...
            if headsup and a_obs is not None:
                likelihood = math.exp(
                    -((primary_strength - a_obs) ** 2)
                    / (2 * self.likelihood_sigma**2)
                )
        ...
        equity = (
            ...
            if weight_sum > 0
            else uniform_win / self.belief_particles
        )
    ...
    def _observed_opponent_strength(self, state: dict[str, Any]) -> float | None:
        ...
        signals = [
            ...
            and event.get("action") in ACTION_STRENGTH_SIGNAL
        ]
        return max(signals) if signals else None

    def _opp_strength(
        self, hidden: Sequence[Card], public: Sequence[Card], state: dict[str, Any]
    ) -> float:
    ...
    # ----------------------------------------------------------------- EV model
    def _action_ev(
        self, action: str, state: dict[str, Any], belief: dict[str, Any]
    ) -> float:
        pot = float(state.get("pot", 0) or 0)
        call = float(state.get("call_amount", 0) or 0)
        ...
        if action == "CHECK":
            ...
            return self._ce(
                [(w, win * pot - invested) for _, w, win in particles], scale
            )
        if action == "CALL":
            if not particles:
                return belief["equity"] * (pot + call) - (invested + call)
            outcomes = [
                (w, win * (pot + call) - (invested + call)) for _, w, win in particles
            ]
    ...
    def _risk_scale(self, state: dict[str, Any]) -> float:
        ...
        total = float(state.get("my_chips", 0) or 0) + float(
            state.get("my_invested", 0) or 0
        )
        if total > 0:
            return total
    ...
    def _ce(self, outcomes: Sequence[tuple[float, float]], scale: float) -> float:
        ...
        soft = (
            sum(weight * math.exp(-lam * (net - ref)) for weight, net in units) / total
        )
        return (ref - math.log(soft) / lam) * scale

    def _raise_ev(
        self, state: dict[str, Any], belief: dict[str, Any], raise_amount: float
    ) -> float:
        ...
        for strength, weight, win in belief["particles"]:
            calls = (
                (not headsup)
                or strength >= 0.90
                or self.opp_model._should_call(strength, facing_state)
            )
    ...
    # -------------------------------------------------------------- discard MC
    def _retained_equity(
        self, retained: Sequence[Card], initial: Sequence[Card]
    ) -> float:
    ...
    # ---------------------------------------------------------------- utilities
    def _opponent_hidden_count(
        self, state: dict[str, Any], public: Sequence[Card]
    ) -> int:
