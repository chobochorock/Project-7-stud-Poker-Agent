...
class ClaudeSearchAgent(ClaudeBeliefBRAgent):
    def __init__(
        ...
        self._cont = ClaudeBeliefBRAgent(
            f"{name}__cont",
            belief_particles=cont_particles,
            aggression_margin=0.40,
            risk_lambda=0.0,
    ...
    def _sample_determinizations(self, state: dict[str, Any], count: int):
        ...
        """
        own = self._cards(state.get("my_hidden_cards", [])) + self._cards(
        ...
        for _ in range(count):
            ...
            opp_discard = draw[c]
            c += 1
            opp_hidden = draw[c : c + opp_hidden_count]
            c += opp_hidden_count
            my_future = draw[c : c + my_future_n]
            c += my_future_n
            opp_future = draw[c : c + opp_future_n]
            c += opp_future_n
            if a_obs is not None:
                strength = self._opp_strength(opp_hidden, opp_public, state)
                weight = math.exp(
                    -((strength - a_obs) ** 2) / (2 * self.likelihood_sigma**2)
                )
            else:
                weight = 1.0
            dets.append(
                (weight, opp_hidden, opp_discard, my_future, opp_future, remaining)
            )
        return dets
    ...
    def _rollout_value(self, state: dict[str, Any], action: str, dets) -> float:
        ...
        for weight, opp_hidden, opp_discard, my_future, opp_future, remaining in dets:
            net = self._one_rollout(
                state, action, opp_hidden, opp_discard, my_future, opp_future, remaining
            )
            value += weight * net
            total_w += weight
    ...
    def _one_rollout(
        self, state, action, opp_hidden, opp_discard, my_future, opp_future, remaining
    ) -> float:
        game, me, opp = self._reconstruct(
            state, opp_hidden, opp_discard, my_future, opp_future, remaining
        )
    ...
    # --------------------------------------------------------- reconstruction
    def _reconstruct(
        self, state, opp_hidden, opp_discard, my_future, opp_future, remaining
    ):
        ...
        game = PokerGame(
            names, log_file=None, ante=ante, game_mode=mode, ev_stack_ante=ev_ante
        )
    ...
    def _acted_this_street(self, game, player) -> bool:
        ...
        return any(
            e.get("actor_index") == idx and e.get("street") == game.street
    ...
    def _finish_current_round(self, game, agents, me, opp, my_was_raise) -> None:
        ...
        if my_was_raise:
            ...
            if (
                game.current_highest_bet - me.current_bet == 0
                and opp_matched
                and self._acted_this_street(game, opp)
            ):
                pending = set()
        ...
        while pending and guard < 16:
            ...
            if not valids:
                pending.discard(player)
                continue
            act = agents[player.name].choose_action(
                game.get_ai_state(player, valids), valids
            )
            ...
            if player.is_folded or self._alive(game) <= 1:
                return
            pending = (
                {me if player is opp else opp} if was_raise else (pending - {player})
            )
