...
class UniformRolloutAgent(PokerAgent):
    ...
    def choose_action(
        ...
        self.trajectory.append(
            (canonical_state(state), action, state["street"], state["raise_count"])
        )
        return action
    ...
    def choose_discard_and_reveal(self, hidden_cards: Sequence[Any]) -> tuple[int, int]:
        ...
        reveal = self.rng.choice(
            [index for index in range(len(hidden_cards)) if index != discard]
        )
...
class RolloutTable:
    ...
    def flush(
        ...
        with self.connection:
            self.connection.executemany(
                ...
                """,
                (
                    (state, action, int(values[0]), values[1])
                    for (state, action), values in q_updates.items()
                ),
            )
            self.connection.executemany(
                """
                ...
                """,
                (
                    (street, depth, action, visits)
                    for (street, depth, action), visits in coverage_updates.items()
                ),
            )
        q_updates.clear()
    ...
    def finish(self, metadata: dict[str, Any]) -> tuple[int, int]:
        ...
        rows = int(
            self.connection.execute("SELECT COUNT(*) FROM q_values").fetchone()[0]
        )
        samples = int(
            self.connection.execute(
                "SELECT COALESCE(SUM(visits), 0) FROM q_values"
            ).fetchone()[0]
        )
...
def canonical_state(state: dict[str, Any]) -> str:
    ...
    history = [
        [
            STREETS[event["street"]],
            0 if event["actor"] == "self" else 1,
    ...
    try:
        while True:
            ...
            if progress_seconds > 0 and now >= next_progress:
                elapsed = now - start
                print(
                    f"{hand_count:,} hands, {sample_count:,} samples, {hand_count / elapsed:,.1f} hands/s"
                )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run uniform stackless heads-up EV rollouts."
    )
    ...
    parser.add_argument(
        "--max-gib", type=float, help="Stop when SQLite plus WAL reaches this size"
    )
    parser.add_argument("--output", type=Path)
    parser.add_argument(
        "--inspect", type=Path, help="Decode and print an existing rollout table"
    )
    parser.add_argument(
        "--limit", type=int, default=5, help="Rows to print with --inspect"
    )
...
def main() -> None:
    ...
    if args.inspect is not None:
        print(
            json.dumps(
                inspect_table(args.inspect, args.limit), ensure_ascii=False, indent=2
            )
        )
        return
    output = (
        args.output
        or Path("data/replays") / f"ev_rollout_{datetime.now():%Y%m%d_%H%M%S}.sqlite3"
    )
    seconds = (
        args.seconds
