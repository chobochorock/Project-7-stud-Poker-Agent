    ...
    *,
    ...
    with contextlib.redirect_stdout(io.StringIO()):
        while samples < max_rollouts:
            ...
            if (
                samples >= min_rollouts
                and epsilon_ante > 0
                and max(radii) <= epsilon_ante
            ):
                break
...
def _conditioned_rollout(
    ...
    *,
    ...
    if not opponent.discard_and_reveal(
        *opponent_agent.choose_discard_and_reveal(opponent.hidden_cards)
    ):
        raise ValueError("Opponent returned an invalid H4 action.")

    agents = {root.name: continuation_agent, opponent.name: opponent_agent}
    for street, is_public in (
        ("4th", True),
        ("5th", True),
        ("6th", True),
        ("7th_hidden", False),
...
def _ci95_radii(
    sums: Sequence[float], squared_sums: Sequence[float], samples: int
) -> list[float]:
    if samples < 2:
...
class H4NodeTable:
    ...
    def add(
        self, estimate: dict[str, Any], model_dir: Path, clusterer: str, decision: str
    ) -> None:
...
def collect_h4_values(
    ...
    *,
    ...
    table.close(
        {**result, "model_dir": model_dir.resolve(), "opponent_policy": "heuristic"}
    )
    return result
...
def main(argv: Sequence[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Collect adaptive H4 action-EV samples."
    )
