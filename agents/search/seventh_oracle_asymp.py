...
def pure_plans(types: int) -> list[PurePlan]:
    ...
    return [
        PurePlan(
            tuple(code & 1 for code in choices),
            ...
            curve.append(
                {
                    "updates": 2 * iteration,
                    "nash_conv": gap(
                        matrix, row_sum / row_sum.sum(), column_sum / column_sum.sum()
                    ),
                }
            )
...
def run_asymp(
    ...
    for iteration in range(1, iterations + 1):
        ...
        column_for_column = project_simplex(
            column_for_column
            + step * (matrix.T @ row_for_column - mu * column_for_column)
        )
...
def behavior_summary(
    mixture: np.ndarray, plans: list[PurePlan]
) -> list[dict[str, float]]:
    return [
        {
            "first_aggressive": float(
                sum(weight * plan.first[type_] for weight, plan in zip(mixture, plans))
            ),
            "call_facing_bet": float(
                sum(
                    weight * plan.response[type_]
                    for weight, plan in zip(mixture, plans)
                )
            ),
        }
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.updates < 4 or args.updates % 4 or min(args.pot, args.bet, args.mu) <= 0:
        parser.error(
            "updates must be a positive multiple of 4; pot/bet/mu must be positive"
        )
    result = run(args)
