    ...
    def test_returns_valid_action(self):
        ...
        with contextlib.redirect_stdout(io.StringIO()):
            action = agent.choose_action(
                make_state(), ["CHECK", "QUARTER", "HALF", "FOLD"]
            )
...
class BeliefUpdateTests(unittest.TestCase):
    def test_aggression_shifts_belief_toward_strong_hands(self):
        ...
        """
        marginal = dict(
            my_hidden_cards=["s8", "d8"], my_public_cards=["h3", "cJ", "s6"]
        )
        ...
        passive = agent.estimate_belief(
            make_state(
                betting_history=[
                    {"street": "6th", "actor": "opponent_1", "action": "CHECK"}
                ],
                **marginal,
            )
        )
    ...
    def test_strong_hand_bets_for_value(self):
        ...
        agent = ClaudeBeliefBRAgent(
            "A", belief_particles=200, aggression_margin=0.0, seed=5
        )
        with contextlib.redirect_stdout(io.StringIO()):
            action = agent.choose_action(
                make_state(), ["CHECK", "QUARTER", "HALF", "FOLD"]
    ...
    def test_aggression_margin_suppresses_thin_bets(self):
        ...
        passive_agent = ClaudeBeliefBRAgent(
            "A", belief_particles=200, aggression_margin=0.8, seed=5
        )
        with contextlib.redirect_stdout(io.StringIO()):
            action = passive_agent.choose_action(
                marginal, ["CHECK", "QUARTER", "HALF", "FOLD"]
            )
...
class BeliefBRIntegrationTests(unittest.TestCase):
    def test_plays_full_ev_hand_zero_sum(self):
        ...
        game = PokerGame(
            ["Player_1", "Player_2"], log_file=None, ante=1000, game_mode="ev"
        )
        with contextlib.redirect_stdout(io.StringIO()):
