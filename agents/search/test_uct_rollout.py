...
class UCTRolloutTests(unittest.TestCase):
    ...
    def test_symmetric_uct_can_record_both_players_tree_nodes(self):
        ...
        root = next(
            record for record in records if record.state_json == records[0].state_json
        )
        ...
        self.assertIn(
            "uct-v3-stack1000-tree", {record.search_version for record in records}
        )
    ...
    def test_collector_writes_separate_uct_nodes(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(
                result["actual_root_simulations"], result["search_roots"] * 8
            )
