...
class _UCTNode:
    ...
    def choose(
        self, rng: random.Random, exploration: float, reward_scale: float
    ) -> str:
        ...
        return max(
            ...
            key=lambda action: (
                ...
                + exploration
                * reward_scale
                * math.sqrt(log_total / self.visits[action]),
                -ACTIONS[action],
            ),
        )
...
class _EVSimulation:
    def __init__(
        ...
        if state.get("game_mode") != "ev" or state.get("seat_count") != 2:
            raise ValueError(
                "UCT simulation currently supports heads-up EV states only."
            )
    ...
    def valid_actions(self) -> list[str]:
        ...
        if can_bet and self.current_highest_bet == 0 and remaining > 0:
            valid.add("BBING")
        if can_bet and self.pot > 0 and remaining > call_amount:
    ...
    def _advance_street(self) -> None:
        while True:
            ...
            self.raise_count = 0
            actors = [
                seat for seat, player in self.players.items() if not player.all_in
            ]
            if len(actors) < 2:
                continue
            self.actor = max(
    ...
    def _record(self, action: str) -> None:
        self.history.append(
            {"street": self.street, "actor": self.actor, "action": action}
        )
...
class UCTPokerAgent(PokerAgent):
    ...
    def __init__(
        ...
        self.search_version = (
            f"{self.SEARCH_VERSION}-stack{DEFAULT_EV_STACK_ANTE}{suffix}"
        )
        ...
        for simulation_index in range(1, self.simulations + 1):
            ...
            while not simulation.terminal:
                ...
                if actor == root_seat or self.opponent_policy == "uct":
                    ...
                    path.append(
                        (actor, node, action, simulation.players[actor].invested)
                    )
                else:
    ...
    def _record(
        ...
        return UCTSearchRecord(
            ...
            return_sums={
                action: node.return_sums.get(action, 0.0) for action in ACTIONS
            },
            return_squared_sums={
