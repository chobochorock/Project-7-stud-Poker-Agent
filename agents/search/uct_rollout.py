...
class UCTNodeTable:
    def __init__(self, path: Path):
        ...
        if "ddadang_visits" not in columns:
            ...
            raise ValueError(
                "UCT database uses obsolete betting rules; use a new shard"
            )
    ...
    def flush(self, records: list[UCTSearchRecord]) -> None:
        ...
        with self.connection:
            self.connection.executemany(
                self._insert_sql, (self._row(record) for record in records)
            )
        records.clear()
...
def run_uct_rollouts(
    ...
    try:
        while hands is None or completed_hands < hands:
            ...
            completed_hands += 1
            collected_searches += (
                sum(agent.searches for agent in agents.values()) - searches_before
            )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Collect heads-up EV UCT search nodes."
    )
    parser.add_argument(
        "--output", type=Path, default=Path("data/replays/uct_rollout.sqlite3")
    )
    ...
    parser.add_argument(
        "--opponent-policy", choices=("random", "uct"), default="random"
    )
