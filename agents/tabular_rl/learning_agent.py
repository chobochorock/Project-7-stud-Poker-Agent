...
class SharedTrajectoryDatabase:
    ...
    def load(self) -> dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(
                f"Cannot load checkpoint; original file preserved: {self.filename}"
            ) from exc
        ...
        database["metadata"]["legacy_entries_ignored"] = (
...
class LearningAgent(PokerAgent):
    ...
    db_filename = "agents/tabular_rl/data/LearningAgent_Shared_db.json"
    ...
    def __init__(
        ...
        if self.db_filename not in type(self)._stores:
            type(self)._stores[self.db_filename] = SharedTrajectoryDatabase(
                self.db_filename
            )
        if self.db_filename not in type(self)._shared_databases:
            type(self)._shared_databases[self.db_filename] = (
                type(self)._stores[self.db_filename].load()
            )
    ...
    def choose_action(
        ...
        if random.random() < self.exploration_rate:
            ...
            valid_values = {
                action: q_entry[action]["value"] for action in valid_actions
            }
            best_value = max(valid_values.values())
            best_actions = [
                action for action, value in valid_values.items() if value == best_value
            ]
    ...
    def observe_reward(
        self, reward: int, final_state: dict[str, Any] | None = None
    ) -> None:
        ...
        for step in self.trajectory:
            ...
            action_value["value"] += (reward - action_value["value"]) / action_value[
                "visits"
            ]
    ...
    def _ensure_q_entry(
        self, state_key: str, valid_actions: Sequence[str]
    ) -> dict[str, Any]:
