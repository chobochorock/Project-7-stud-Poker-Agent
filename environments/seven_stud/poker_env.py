...
def get_public_betting_priority(cards: Sequence[Card]) -> tuple[int, ...]:
    ...
    if groups[0][0] == 3:
        kickers = sorted(
            (value for value in values if value != groups[0][1]), reverse=True
        )
    ...
    if groups[0][0] == 2 and len(groups) > 1 and groups[1][0] == 2:
        ...
        kickers = sorted(
            (value for value in values if value not in {high_pair, low_pair}),
            reverse=True,
        )
    ...
    if groups[0][0] == 2:
        ...
        kickers = sorted(
            (value for value in values if value != pair_value), reverse=True
        )
...
def evaluate_5_cards(cards: Sequence[Card]) -> tuple[int, ...]:
    ...
    if freq[0][0] == 3:
        kickers = sorted(
            (value for value in values if value != freq[0][1]), reverse=True
        )
...
class PokerGame:
    ...
    def __init__(
        ...
        log_file: str | None = "data/runtime/state_log.txt",
        ...
        if ev_stack_antes is not None and (
            ...
            raise ValueError(
                "ev_stack_antes must contain one positive value per player."
            )
    ...
    def get_valid_actions(self, player: Player) -> list[str]:
        ...
        if self.game_mode == "ev":
            ...
            if can_raise and self.current_highest_bet == 0 and remaining > 0:
                valid.append("BBING")
            if can_raise and self.pot > 0 and remaining > call_amount:
            ...
            if (
                ...
                and self.current_highest_bet == 0
            ):
                valid.append("BBING")
    ...
    def get_ai_state(
        self, player: Player, valid_actions: Sequence[str] | None = None
    ) -> dict[str, Any]:
            ...
            opponents.append(
                {
                    ...
                    "effective_stack": (
                        self._ev_stack(opponent) if self.game_mode == "ev" else None
                    ),
        ...
        state = {
            ...
            "my_discarded_card": str(player.discarded_card)
            if player.discarded_card
            else None,
            ...
            "effective_stack_ante": (
                self._ev_stack(player) // self.ante if self.game_mode == "ev" else None
            ),
            "effective_stack": (
                self._ev_stack(player) if self.game_mode == "ev" else None
            ),
            "opponents": opponents,
    ...
    def apply_action(self, player: Player, action: str) -> bool:
        ...
        if action not in valid_actions:
            raise ValueError(
                f"{action} is not valid for {player.name}. Valid actions: {valid_actions}"
            )
    ...
    def resolve_showdown(self) -> dict[str, Any]:
        ...
        contributions = {
            player: player.invested for player in self.players if player.invested > 0
        }
        ...
        for level in levels:
            pot_amount = sum(
                max(0, min(amount, level) - previous_level)
            ...
            print(
                f"  side pot up to {level}: {pot_amount}, winners={payout['winners']}, awards={awards}"
            )

        self.pot = 0
        for player in self.players:
            player.is_eliminated = (
                False if self.game_mode == "ev" else player.chips <= 0
            )
    ...
    def play_hand(self, active_agents: dict[str, BasePokerAgent]) -> dict[str, Any]:
        missing_agents = [
            player.name for player in self.players if player.name not in active_agents
        ]
        if missing_agents:
        ...
        for street_name, is_public in streets:
            ...
            if (
                street_name != "4th"
                and len([player for player in survivors if player.can_act()]) >= 2
            ):
        ...
        for player in self.players:
            ...
            print(
                f"{player.name}: {player.chips} chips, invested {player.invested}, status={status}"
            )
        return result

    def _commit_chips(
        self, player: Player, requested_amount: int, count_for_round: bool
    ) -> int:
    ...
    def _history_from_view(self, viewer_index: int) -> list[dict[str, Any]]:
        ...
        for event in self.betting_history:
            ...
            public_event = {
                key: value for key, value in event.items() if key != "actor_index"
            }
